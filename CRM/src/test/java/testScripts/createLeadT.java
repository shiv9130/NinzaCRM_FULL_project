...
public class createLeadT extends baseClass {
	...
	public void createLeadTest() {
		...
		String rs = ju.generateRandomAlphabet();
       ...
       String e= ju.generateRandomEmail();
       System.out.println(e);
		String leadName = eu.toReadDataFromExcel("lead", 1, 0);
		cl.getTxtFLeadName().sendKeys(leadName + rs);
		...
		/*
		 * window.remove(winP); for(String WinId:window) {
		 * driver.switchTo().window(WinId);
		 * 
		 * try { Thread.sleep(5000); } catch (InterruptedException e) { // TODO
		 * Auto-generated catch block e.printStackTrace(); }
		 * 
		 * WebElement id = driver.findElement(By.id("search-criteria")); Select s = new
		 * Select(id); s.selectByVisibleText("Campaign ID");
